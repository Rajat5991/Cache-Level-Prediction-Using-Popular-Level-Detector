...
uint32_t MESIBottomCC::getParentId(Address lineAddr) {
    ...
    //std::cout << "CC: LineAddr " << tmp  << std::endl;
    //std::cout << "CC: LineAddr : " << std::hex << tmp  << std::endl;
    for (uint32_t i = 0; i < 4; i++) {
        res ^= (uint32_t) ( ((uint64_t)0xffff) & tmp);
    //std::cout << "CC: LineAddr res: " << std::hex << res  << std::endl;
        tmp = tmp >> 16;
    }
    //std::cout << "CC: LineAddr res parents.size(): " << std::hex << res % parents.size()  << std::endl;
...
uint64_t MESIBottomCC::processEviction(Address wbLineAddr, uint32_t lineId, bool lowerLevelWriteback, uint64_t cycle, uint32_t srcId) {
    ...
    if (lowerLevelWriteback) {
      ...
      skip_l2_ref_cache = 0; // reset so that access method of parent can lookpup the cache.
      skip_l3_ref_cache = 0; // reset so that access method of parent can lookpup the cache.
...
uint64_t MESIBottomCC::processAccess(Address lineAddr, uint32_t lineId, AccessType type, uint64_t cycle, uint32_t srcId, const g_string& name, uint32_t flags) {
    uint64_t respCycle = cycle;
    MESIState* state = &array[lineId];
    std::string level_name = name.c_str();
    switch (type) {
        ...
        case GETS:
            if (*state == I) {
               ...
               //level_predictor 
               //NOTE :- We are not skipping mem level to prevent from data loss in case we predicted wrong
               ...
               counters["l2_ref_counter"] = l2_ref_counter;
               counters["l3_ref_counter"] = l3_ref_counter;
               counters["mem_ref_counter"] = mem_ref_counter;
              // Create a vector of pairs from the map
               std::vector<std::pair<std::string, int>> sorted_vector(counters.begin(), counters.end());
             //Sort the vector based on values in descending order
               std::sort(sorted_vector.begin(), sorted_vector.end(), [](const std::pair<std::string, int>& a, const std::pair<std::string, int>& b) {
           ...
           //  Use key and value as needed 
               ...
               if (level_name.find("l1") != std::string::npos)
                 {
                   if((first > THRESHOLD) && (FIRST == "l3_ref_counter"))
                      skip_l2_ref_cache = 1;
                                            
                  else if((first > THRESHOLD) && (FIRST == "mem_ref_counter"))
                      skip_l2_ref_cache = 1;

                  else if(first < THRESHOLD)// access first and second highest popular mem/l3 level and ignore l2 -> when highest counter less than threshold then two level lookup 
                      {
                        if(THIRD == "l2_ref_counter")
                          skip_l2_ref_cache = 1;
                       }
                 }
                else if (level_name.find("l2") != std::string::npos)
                 {
                   if((first > THRESHOLD) && (FIRST == "l2_ref_counter"))
                   skip_l3_ref_cache = 1;
                   if((first > THRESHOLD) && (FIRST == "mem_ref_counter"))
                   skip_l3_ref_cache = 1;
                   else if(first < THRESHOLD) // access first and second highest mem/l2 level and ignore l3 -> when highest counter less than threshold then two level lookup 
                      {
                        if(THIRD == "l3_ref_counter")
                          skip_l3_ref_cache = 1;
                       }
            ...
            } else {  // this part is exexuted only if there is a hit
                profGETSHit.inc();
            
                if(/*lineId != -1 && */(level_name.find("l2") != std::string::npos)) 
                 {
                   l2_ref_counter++;
                   if(l3_ref_counter != 0)
                      l3_ref_counter--;
                   if(mem_ref_counter != 0)
                      mem_ref_counter--;
                 }
     
                else if(/*lineId != -1 && */ (level_name.find("l3") != std::string::npos))
                 {
                  l3_ref_counter++; 
                  if(l2_ref_counter != 0)
                     l2_ref_counter--;
                  ...
                  if(/*lineId != -1 && */(level_name.find("mem") != std::string::npos))
                    {
                     mem_ref_counter++; // If main memory is accessed 
                     if(l3_ref_counter != 0)
                      l3_ref_counter-- ;
                     if(l2_ref_counter != 0)
                      l2_ref_counter--;
        ...
        case GETX:
            if (*state == I || *state == S) {
                ...
                //std::cout << "GETX - ParentId: " << parentId << ", Parent: " << parents[parentId]->getName() << std::endl;
                //level_predictor 
               //NOTE :- We are not skipping mem level to prevent from data loss in case we predicted wrong
                //std :: vector<int> level_counters = {l2_ref_counter, l3_ref_counter, mem_ref_counter};
                /*std::vector<int> level_counters = {
                  static_cast<int>(l2_ref_counter),
                  static_cast<int>(l3_ref_counter),
                  static_cast<int>(mem_ref_counter)
                };*/
               std::map<std::string, int> counters;
              // std::string level_name = name.c_str();
               //std::map<std::string, string> level_name;
               std::string FIRST;
               std::string SECOND;
               std::string THIRD;
               //std::map<std::string, std::string> level_counters;
               int first = 0 /*second,third*/;
              // first = 0;
               //second = 0;
               //third = 0;
               /* if (first + second + third == 0) {
    //Some meaningful operation involving the variables
            ...
            // Iterate over the sorted vector
              for (const auto& pair : sorted_vector) {
                 ...
                 if(i == 0)
                  ...
                  {
                   // level_counters["second"] = value;
                    //second = value;
                    SECOND = key;
                  }
                 else
                  {
                    //level_counters["third"] = value;
                    //third = value;
                    THIRD = key;
                  }
                 i++;
           //  Use key and value as needed 
               ...
               if (level_name.find("l1") != std::string::npos)
                 {
                  // std::cout << "level_predictor: level_counters - first" << first  << std::endl;
                  // std::cout << "level_predictor: level_name " << FIRST  << std::endl;
                   if((first > THRESHOLD) && (FIRST == "l3_ref_counter"))
                      skip_l2_ref_cache = 1;
                ...
                profGETXHit.inc();
                 ...
                 std::cout << "CC_CPP level_name:" << level_name << std::endl;
                //std::cout << "CC_CPP: lineId" << lineId  << std::endl;
                if(/*lineId != -1 && */(level_name.find("l2") != std::string::npos)) 
                 {
                   l2_ref_counter++;
                   if(l3_ref_counter != 0)
              ...
              //  std::cout << "CC_CPP: l2_ref_counter::" << l2_ref_counter  << std::endl;
               // std::cout << "CC_CPP: l3_ref_counter::" << l3_ref_counter  << std::endl;
               // std::cout << "CC_CPP: mem_ref_counter::" << mem_ref_counter  << std::endl;
