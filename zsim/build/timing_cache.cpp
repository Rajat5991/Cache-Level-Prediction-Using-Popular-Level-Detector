    ...
    : Cache(_numLines, _cc, _array, _rp, _accLat, _invLat, _name), numMSHRs(mshrs), tagLat(_tagLat), ways(_ways), cands(_cands)
{
    ...
    assert(numMSHRs > 0);
    activeMisses = 0;
    //l2_l3_mem_counter = 0; //Initializing l2 l3 and mem level counter to 0
    domain = _domain;
...
uint64_t TimingCache::access(MemReq& req) { // predLineAddr declared argument for level predictor and parent identification 
    ...
    if (likely(!skipAccess)) {
        ...
        //levelPredictor(Address predLineAddr);
        //std::cout << "Timing_Cache cache_name:" << name << std::endl;
        //std::cout << "Timing_Cache skip_l3_ref_cache::" << skip_l3_ref_cache  << std::endl;
        //std::cout << "Timing_Cache skip_l2_ref_cache::" << skip_l2_ref_cache  << std::endl; 
        if((skip_l2_ref_cache || skip_l3_ref_cache) != 1) // skip tag matching based on popular cache level
           lineId = array->lookup(req.lineAddr, &req, updateReplacement);
        else
          lineId = -1;
        trace(Cache, "[%s] Debug Cache name", name.c_str());
        ...
        //cache_name = name.c_str();
        respCycle = cc->processAccess(req, lineId, respCycle, name, &getDoneCycle);
...
/* 
uint64_t TimingCache::levelPredictor(Address predLineAddr){

string parentName = parents[getParentId(predLineAddr)]->getName(); // get the parent Name
std :: vector<int> level_counters = {l2_ref_counter, l3_ref_counter, mem_ref_counter};
std :: sort(level_counters.begin(), level_counters.end(), std :: greater<int>()); // sorting counter in decreasing order

 if((level_counters[0] > THRESHOLD) && (level_counters[0] == l2_ref_counter) && parentName.find("l2") != std::string::npos)
  {
    
   

  || l3_ref_counter >= THRESHOLD || mem_ref_counter >= THRESHOLD) 
   {
