...
class CC : public GlobAlloc {
    public:
        ...
        virtual uint64_t processAccess(const MemReq& req, int32_t lineId, uint64_t startCycle, const g_string& _name, uint64_t* getDoneCycle = nullptr) = 0;
...
class MESIBottomCC : public GlobAlloc {
    ...
    public:
        ...
        uint64_t processAccess(Address lineAddr, uint32_t lineId, AccessType type, uint64_t cycle, uint32_t srcId, const g_string& _name, uint32_t flags);
...
class MESICC : public CC {
    ...
    public:
        ...
        void setParents(uint32_t childId, const g_vector<MemObject*>& parents, Network* network) {
           // std::cout << "setParents - Parents: ";
            //std::cout << ", Cache Name: " << name.c_str() << ", ChildId: " << childId << std::endl;
            bcc = new MESIBottomCC(numLines, childId, nonInclusiveHack);
            bcc->init(parents, network, name.c_str());
       } 

        void setChildren(const g_vector<BaseCache*>& children, Network* network) {
            //std::cout << "setChildren - Children: ";
            //std::cout << ", Cache Name: " << name.c_str() << std::endl;
        ...
        uint64_t processAccess(const MemReq& req, int32_t lineId, uint64_t startCycle, const g_string& name, uint64_t* getDoneCycle = nullptr) {
            ...
            if (lineId == -1 || (((req.type == PUTS) || (req.type == PUTX)) && !bcc->isValid(lineId))) { //can only be a non-inclusive wback
                ...
                respCycle = bcc->processAccess(req.lineAddr, lineId, req.type, startCycle, req.srcId, name, flags);
...
class MESITerminalCC : public CC {
    ...
    public:
        ...
        uint64_t processEviction(const MemReq& triggerReq, Address wbLineAddr, int32_t lineId, uint64_t startCycle) {
            bool lowerLevelWriteback = false;
            //std::cout << "Cache_coh_h: wbLineAddr:" << wbLineAddr << std::endl;
        ...
        uint64_t processAccess(const MemReq& req, int32_t lineId, uint64_t startCycle, const g_string& name, uint64_t* getDoneCycle = nullptr) {
            ...
            assert(!getDoneCycle);
            //if needed, fetch line or upgrade miss from upper level
            uint64_t respCycle = bcc->processAccess(req.lineAddr, lineId, req.type, startCycle, req.srcId, name, req.flags);
            //at this point, the line is in a good state w.r.t. upper levels
            return respCycle;
        }
