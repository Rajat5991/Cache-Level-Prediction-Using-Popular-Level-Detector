...
BaseCache* BuildCacheBank(Config& config, const string& prefix, g_string& name, uint32_t bankSize, bool isTerminal, uint32_t domain) {
    ...
    CC* cc;
        ...
        std::cout << "isTerminal is true" << std::endl;
    } else {
        cc = new MESICC(numLines, nonInclusiveHack, name);
        std::cout << "isTerminal is false" << std::endl;
    }
    rp->setCC(cc);
    if (!isTerminal) {
        if (type == "Simple") {
            ...
            std::cout << "mshrs " << mshrs << std::endl;
            std::cout << "tagLat " << tagLat << std::endl;
            std::cout << "timingCandidates " << timingCandidates << std::endl;
            cache = new TimingCache(numLines, cc, array, rp, accLat, invLat, mshrs, tagLat, ways, timingCandidates, domain, name);
...
static void InitSystem(Config& config) {
    ...
    for (const char* grp : cacheGroupNames) {
        std::cout << "Cache Group: " << grp << std::endl;
        string group(grp);
        ...
        // Print group, children, and childMap[group]
   ...
   auto isTerminal = [&](string group) -> bool {
    ...
    //uint32_t isTerminal = config.get<uint32_t>(prefix + "isTerminal", 1);
    ...
    while (!fringe.empty()) {
        ...
        cMap[group] = BuildCacheGroup(config, group, isTerminal(group)); // creates cache group [[l1i-0 ,l2i-0, ....], (l2-0, l2-1, l2-2.....), (llc-b0...)]
        std::cout << "Group: " << group << ", CacheGroup: " << cMap[group] << std::endl; 
    ...
    //Build the memory controllers:w
    ...
    uint32_t childId = 0;
    for (BaseCache* llcBank : (*cMap[llc])[0]) { //line:429 -> cg[i][j] i = number of cache and j= number of banks and cg[i][j] points to cMap[llc]
        llcBank->setParents(childId++, mems, network); //setParents for l3-0b0 ...l3-0b7
        trace(llcBankFromInit, "%d Debug ChildId", childId);
        ...
        for (uint32_t p = 0; p < parents; p++) {
            ...
            for (uint32_t c = p*childrenPerParent; c < (p+1)*childrenPerParent; c++) {
                for (BaseCache* bank : childCaches[c]) {
                  std::cout << "ChildId: " << childId << ", ChildCache: " << bank->getName() << std::endl; 
            ...
            if (printHierarchy) {
                ...
                std::cout << "ParentCaches[" << p << "]: ";
           for (BaseCache* bank : parentCaches[p]) {
             std::cout << bank->getName() << " ";
            }
            std::cout << std::endl;
            for (BaseCache* bank : parentCaches[p]) {
